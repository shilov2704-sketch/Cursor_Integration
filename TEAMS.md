...
1. Сохраните. Нужен **новый** прогон агента.
...
В баге: шаблон WEB/Backend/МП, теги `DEV` + клиент + `Create Cursor agent`, Assign пустой, фактический и ожидаемый результат, **ссылка на этот тред Teams** в описании, вложения (фото/видео) из треда.

Чтобы ссылка на тред точно попала в баг, в сообщение можно вставить **Copy link** / **Копировать ссылку** с корневого сообщения треда. Если скриншоты не прикрепились к work item — пришлите файлы follow-up `@Cursor`.
